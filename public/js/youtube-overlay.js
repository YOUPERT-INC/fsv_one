...
(function (moduleFunc) {
  ...
  cpo.createPlayerContainer = function() {
    ...
    if($($this.overlayContainer).length === 0) {
      var o = $('<div class="videoPlayerOverlay hide hiddenTransform" id="youtubePlayerOverlay" data-hasloaded="false"></div>'),
          cButton = $('<button id="youtubeOverlayCloser" class="defaultButton closeIcon"><img src="js/x-mark-black-icon.svg" alt="Close" title="close"/></button>'),
          b = $("body");
      cButton.appendTo(o);
      o.appendTo(b);
    }
  ...
  cpo.activateOnClickHandler = function() {
    ...
    //$(t).on("click", function() {
      ...
      if(idEquals) {
        ...
        var requiredIframe = $('<iframe width="100%" height="100%" frameborder="0" allow="autoplay; encrypted-media; picture-in-picture;" controls controlsList="nodownload" allowfullscreen mozallowfullscreen webkitallowfullscreen></iframe>');
        requiredIframe.attr({
  ...
  cpo.closePlayerOverlay = function() {
    var $this = this,
        o = $this.overlayContainer;
    //$(o).find("iframe").remove();
    $("body").removeClass("popup-videoplayer");
